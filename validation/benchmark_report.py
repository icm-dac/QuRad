"""Summarise the headless benchmark (validation/results/benchmark) into a table and a figure."""
import glob, json, os
...
B = os.environ.get('QURAD_BENCHMARK', os.path.join(os.path.dirname(__file__), 'results', 'benchmark'))
OUT = os.path.join(os.environ.get('QURAD_RESULTS', os.path.join(os.path.dirname(__file__), 'results')), 'summary'); os.makedirs(OUT, exist_ok=True)
...
fig.tight_layout(pad=0.4, w_pad=1.2); fig.savefig(os.path.join(OUT, 'figure_S3_benchmark.png'), dpi=st.DPI); fig.savefig(os.path.join(OUT, 'figure_S3_benchmark.pdf'))
