...
Inputs: validation/results/{breast,puma/*,synthetic/*} produced by HeadlessRunner and pyradiomics_extract.py.
Outputs (validation/results/summary/): table_S1_feature_agreement.csv/.tex, table_S2_synthetic.csv, figure_S1_agreement.png,
...
R = os.environ.get('QURAD_RESULTS', os.path.join(os.path.dirname(__file__), 'results'))
OUT = os.path.join(R, 'summary'); os.makedirs(OUT, exist_ok=True)
...
def pooled_puma():
    qs, ps = [], []
    for d in sorted(glob.glob(os.path.join(R, 'puma', 'training_set_primary_roi_*'))):
...
def main():
    breast_df, breast_m = compare(os.path.join(R, 'breast', 'qurad.csv'), os.path.join(R, 'breast', 'pyradiomics_noweighting.csv'))
    breast_def, _ = compare(os.path.join(R, 'breast', 'qurad.csv'), os.path.join(R, 'breast', 'pyradiomics_default.csv'))
    ...
    rows = []
    for d in sorted(glob.glob(os.path.join(R, 'synthetic', '*/'))):
        case = os.path.basename(d.rstrip('/'))
        q = pd.read_csv(d + 'qurad.csv'); p = pd.read_csv(d + 'pyradiomics_noweighting.csv')
        gj = json.load(open(os.path.join(os.path.dirname(__file__), 'synthetic', f'{case}.geojson')))
    ...
    fig.tight_layout(pad=0.4); fig.savefig(os.path.join(OUT, 'figure_S1_agreement.png'), dpi=st.DPI); fig.savefig(os.path.join(OUT, 'figure_S1_agreement.pdf')); plt.close(fig)
    ...
    fig, axes = plt.subplots(3, 6, figsize=(st.DOUBLE_COLUMN_IN, 5.4))
    for k, f in enumerate(reps):
        ...
        ax.text(0.03, 0.97, f.replace('_', ' ') + f"\nCCC = {breast_df[breast_df.feature == f].ccc.iloc[0]:.4f}", transform=ax.transAxes, fontsize=6, va='top')
        ...
        ax.text(0.03, 0.97, f"bias = {sci(np.mean(d))}\nSD = {sci(np.std(d))}", transform=ax.transAxes, fontsize=6, va='top')
    ...
    fig.tight_layout(pad=0.3, w_pad=0.4, h_pad=0.6); fig.savefig(os.path.join(OUT, 'figure_S2_bland_altman.png'), dpi=st.DPI); fig.savefig(os.path.join(OUT, 'figure_S2_bland_altman.pdf')); plt.close(fig)
